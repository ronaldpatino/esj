    ...
    $.fn.extend({
        infiniteCarousel: function(options)
        {
            var defaults =
            {
                ...
                thumbnailType: 'none', // buttons, gc, numbers, count, or none
                easeLeft: 'linear',
                easeRight: 'linear',
                imagePath: '/js/infinitecarousel/gc/',
                inView: 1,
                margin: 0,
                advance: 1,
                customClass: null,
            ...
            return this.each(function() {
                ...
                $('li',obj).each(function(index){ // populate arrays
                    ...
                    itemSources.push( $(this).find('img, iframe').attr('src') ); // finds gc and youtube and vimeo iframe sources
                });

                // Check for improper values in inview and advance
                if(o.inView > numItems) o.inView = numItems; // Prevent trying to view more gc than given
                if(o.advance > o.inView) o.advance = o.inView; // Prevent advancing more than inView gc at a time

                // Dimensions should be set explicitly on the items so that we don't have to defer loading code until the gc are loaded
                ...
                // If nav outside carousel, wrap carousel in a div and set padding to compensate for nav. also dont animate nav if outside gc
                ...
                if(o.thumbnailType != 'none')
                {
                    ...
                    {
                        // insert gc in the ic_button divs with width/height set to 100%. need to set width and height on .ic_button in css
